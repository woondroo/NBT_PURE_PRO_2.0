...
class CRedisList extends CRedis 
{
	 ...
	 * @return bool
	 */
	public function getRange( $_key = '' , $_where = self::REDIS_LIST_FRONT , $_count = 20 )
	{
		$key = $this->calculateKey( $_key );

		// 获得索引位置
		$intIndex = $_where == self::REDIS_LIST_FRONT ? 1 : -1;
		...
		// 根据指定的链表位置，获得数据
		if ( self::getConnection() )
			$returnData = self::getConnection()->lRange( $key , $intStart , $intEnd );
	 ...
	 *
	 * @param string $_key		给定的 redis key
	 * @param int $_intStart	开始位置
	 * @param int $_intEnd		结束位置
	 * @return bool
	 */
	public function trimRange( $_key = '' , $_intStart = 0 , $_intEnd = -1 )
	{
		$key = $this->calculateKey( $_key );

		// 裁剪数据
		if ( self::getConnection() )
			$returnData = self::getConnection()->lTrim( $key , $_intStart , $_intEnd );
	...
	public function remove( $_key = '' , $_value = '' , $_count = 1 )
	{
		...
		// 从 List 中移除一个元素
		if ( self::getConnection() )
			$removeResult = self::getConnection()->lRem( $key , $_value , $_count );
	 ...
	 * @return int
	 */
	public function listLength( $_key = '' )
	{
		$key = $this->calculateKey( $_key );

		// 裁剪数据
		if ( self::getConnection() )
			$returnData = self::getConnection()->lLen( $key );
