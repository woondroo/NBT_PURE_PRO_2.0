<?php
class CMongoDocument extends CMongoStructure
{
    ...
    /**
     * @var CMongoOperator Modification operator instance
     */
    private $_operator;
    ...
    /**
     * @param CMongoCollection $collection instance of Mongo collection
     * @param array $data mongo document 
     * @param array $options options of object initialization
     */
    public function __construct(CMongoCollection $collection, array $data = null, array $options = array())
    {
    ...
    {
        $this->_operator = new CMongoOperator;
    ...
    public function __call($name, $arguments) {
        ...
        throw new Exception('CMongoDocument has no method "' . $name . '"');
    }
    ...
    {
        if(!$this->isValid()) {
            throw new Exception('CMongoDocument not valid');
        }
    }
    ...
    public function attachBehaviors(array $behaviors)
    {
        foreach($behaviors as $name => $behavior) {
            ...
            if(!($behavior instanceof CMongoBehavior)) {
                if(empty($behavior['class'])) {
                    throw new Exception('CMongoBehavior class not specified');
                }
    ...
    public function attachBehavior($name, CMongoBehavior $behavior)
    {
    ...
    public function push($fieldName, $value)
    {
        ...
        if($value instanceof CMongoStructure) {
            $value = $value->toArray();
        }
     ...
     * 
     * @param string $fieldName
     * @param integer|string|array|CMongoExpression $expression
     * @return CMongoDocument
     */
