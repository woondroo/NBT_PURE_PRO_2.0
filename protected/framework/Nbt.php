...
/**
	...
	{
		$explodeSpace = explode( "\\" , $className );
		if ( count( $explodeSpace ) > 1 && in_array( $explodeSpace[0] , array('Mdanter') ) )
		{
			array_shift( $explodeSpace );
			array_shift( $explodeSpace );
			$className = NBT_PATH.DIRECTORY_SEPARATOR.'phpecc'.DIRECTORY_SEPARATOR.implode( DIRECTORY_SEPARATOR , $explodeSpace );
		}
		else if ( count( $explodeSpace ) > 1 && in_array( $explodeSpace[0] , array('FG') ) )
		{
			array_shift( $explodeSpace );
			array_shift( $explodeSpace );
			$className = NBT_PATH.DIRECTORY_SEPARATOR.'ASN1'.DIRECTORY_SEPARATOR.implode( DIRECTORY_SEPARATOR , $explodeSpace );
		}
		else if ( count( $explodeSpace ) > 1 )
		{
			array_shift( $explodeSpace );
			$className = NBT_PATH.DIRECTORY_SEPARATOR.'telehash'.DIRECTORY_SEPARATOR.implode( DIRECTORY_SEPARATOR , $explodeSpace );
		}
	...
	public static function setIncludePath()
	{
		...
		$aryIncludePath[] = NBT_PATH.DIRECTORY_SEPARATOR.'mongo';
